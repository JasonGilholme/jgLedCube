// todo: use SECTION and BDD macros to produce more apt test cases
...
TEST_CASE( "SetLed", "[codec]" ) {
    ...
    REQUIRE(decodedCommandID == LED_CUBE_CMD_SET_LED);
    REQUIRE(decodedArgs[0] == 2);
    REQUIRE(decodedArgs[1] == 4);
    REQUIRE(decodedArgs[2] == 6);
    REQUIRE(decodedArgs[3] == 15);
    REQUIRE(decodedArgs[4] == 13);
    REQUIRE(decodedArgs[5] == 9);
}


TEST_CASE( "GetLed", "[codec]" ) {
    // Encode the command
    uint8_t encodedCommand[LED_CUBE_COMMAND_PACKET_SIZE] = {};
    ...
    // Check that the decoded args match the inputs when created.
    REQUIRE(decodedCommandID == LED_CUBE_CMD_GET_LED);
    REQUIRE(decodedArgs[0] == 3);
    REQUIRE(decodedArgs[1] == 6);
    REQUIRE(decodedArgs[2] == 9);
}

//TEST(Serial, EncodeGetLedReturn) {
TEST_CASE( "GetLedReturn", "[codec]" ) {
    // Encode the command
    uint8_t encodedCommand[LED_CUBE_COMMAND_PACKET_SIZE] = {};
    ...
    // Check that the decoded args match the inputs when created.
    REQUIRE(decodedCommandID == LED_CUBE_CMD_GET_LED_RETURN);
    REQUIRE(decodedArgs[0] == 14);
    REQUIRE(decodedArgs[1] == 12);
    REQUIRE(decodedArgs[2] == 13);
}

//TEST(Serial, EncodeGetConfig) {
TEST_CASE( "GetConfig", "[codec]" ) {
    // Encode the command
    uint8_t encodedCommand[LED_CUBE_COMMAND_PACKET_SIZE] = {};
    ...
    // Check that the decoded args match the inputs when created.
    REQUIRE(decodedCommandID == LED_CUBE_CMD_GET_CONFIG);
}
...
TEST_CASE( "Clear", "[codec]" ) {
    ...
    // Check that the decoded args match the inputs when created.
    REQUIRE(decodedCommandID == LED_CUBE_CMD_CLEAR);
}
...
TEST_CASE( "Transport", "[codec]" ) {
    ...
    // Interrogate the output buffer
    REQUIRE(outTransportCommand[0] == jgLedCube::serial::transportSB);
    REQUIRE(outTransportCommand[1] == jgLedCube::serial::transportSB);

    REQUIRE(outTransportCommand[2] == encodedCommand[0]);
    REQUIRE(outTransportCommand[3] == encodedCommand[1]);
    REQUIRE(outTransportCommand[4] == encodedCommand[2]);
    REQUIRE(outTransportCommand[5] == encodedCommand[3]);

    REQUIRE(outTransportCommand[6] == (uint8_t)~(uint8_t)encodedCommand[0]);
    REQUIRE(outTransportCommand[7] == jgLedCube::serial::transportEB);
}
